...
let MensajesController = class MensajesController {
    constructor(mensajeServices) {
        this.mensajeServices = mensajeServices;
    }
    create(createMensageDto, response) {
    ...
    update(updateMensajeDto, response, idMensaje) {
        this.mensajeServices.updateMensaje(idMensaje, updateMensajeDto).then(mensaje => {
            ...
            response.status(common_1.HttpStatus.FORBIDDEN).json({ mensaje: 'error en la edición del mensaje' });
        });
    }
    delete(response, idMensaje) {
        this.mensajeServices.deleteMensaje(idMensaje).then(res => {
            ...
            response.status(common_1.HttpStatus.FORBIDDEN).json({ mensaje: 'Error en la eliminación del mensaje' });
        });
    }
};
__decorate([
...
__decorate([
    ...
    __metadata("design:returntype", void 0)
], MensajesController.prototype, "getOne", null);
__decorate([
    common_1.Put(':id'),
